...
class SparseSRSTE(autograd.Function):
    ...
    @staticmethod
    def forward(ctx, weight, N, M, sparsity_rate = 0.0, decay = 0.0002):
...
class StepDecay(autograd.Function):
    ...
    @staticmethod
    def forward(ctx, weight, N, M, sparsity_rate = 0.0):
        ctx.save_for_backward(weight)
...
## @amir, is this accurate? 
    ...
    def backward(ctx, grad_output):
        weight, = ctx.saved_tensors
        return grad_output * weight, None, None, None
...
class LinearDecay(autograd.Function):
    ...
    @staticmethod
    def forward(ctx, weight, N, M, sparsity_rate = 0.0, linear_decay_coef = 0.01 ,current_step_num = 100):
        ctx.save_for_backward(weight)
...
## @amir, is this accurate? 
    ...
    def backward(ctx, grad_output):
        weight, = ctx.saved_tensors
        return grad_output * weight, None, None, None, None, None
...
class ExponentialDecay(autograd.Function):
    ...
    @staticmethod
    def forward(ctx, weight, N, M, sparsity_rate = 0.0, exp_decay_coef = 0.01 ,current_step_num = 100):
        ctx.save_for_backward(weight)
...
class SparseLinear(nn.Linear):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
        ...
        self.dense_steps = 0
        # self.fine_tune_steps = sparseConfig.fine_tune_steps
    ...
    def get_sparse_weights(self):
        if(self.sparsity_type.lower() == "srste"):
            ...
            if(self.decay_type.lower() == "step"):
                ...
                return LinearDecay.apply(self.weight, self.N, self.M, self.sparsity_rate, self.decay_coef,(self.current_step_num-self.dense_steps))
            elif(self.decay_type.lower() == "exp"):
                return ExponentialDecay.apply(self.weight, self.N, self.M, self.sparsity_rate, self.decay_coef,(self.current_step_num-self.dense_steps))
            else:
